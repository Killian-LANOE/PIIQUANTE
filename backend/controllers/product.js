    ...
    Sauce.findOne({ _id: req.params.id })
        .then(sauce => {
            ...
            if (sauce.usersLiked.indexOf(req.body.userId) != -1 && req.body.like == 0) {
                const userLike = sauce.usersLiked.findIndex(user => user === req.body.userId);
                sauce.usersLiked.splice(userLike, 1);
                sauce.likes -= 1;
            };

            if (sauce.usersDisliked.indexOf(req.body.userId) != -1 && req.body.like == 0) {
                const userLike = sauce.usersDisliked.findIndex(user => user === req.body.userId);
                sauce.usersDisliked.splice(userLike, 1);
                sauce.dislikes -= 1;
            }
            sauce.save();
